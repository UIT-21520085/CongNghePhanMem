Create database DKMHandTHUHP
--drop database DKMHandTHUHP
use DKMHandTHUHP

set dateformat dmy

delete from DS_PHIEUTHU
delete from CT_PHIEUDK
delete from DS_PHIEUDKHP
delete from LOGIN_SINHVIEN
delete from HSSV
delete from HUYEN
delete from TINH

--Danh sach cac tinh, huyen, cac doi tuong uu tien--
create table TINH
(
...
alter table TINH
alter column TenTinh nvarchar(20) COLLATE Vietnamese_CI_AS not null

create table HUYEN
(
	MaHuyen char(4),
	MaTinh char(4) foreign key references TINH(MaTinh),
...
alter table HUYEN
alter column TenHuyen nvarchar(20) COLLATE Vietnamese_CI_AS not null
...
alter table KHOA
alter column TenKhoa nvarchar(30) COLLATE Vietnamese_CI_AS not null

--Danh sach cac Nganh hoc--
create table NGANH
(
	MaNganh char(4) primary key,
	TenNganh nvarchar(30) COLLATE Vietnamese_CI_AS not null,
	MaKhoa char(4) foreign key references KHOA(MaKHoa)
)
alter table NGANH
alter column TenNganh nvarchar(30) COLLATE Vietnamese_CI_AS not null
...
create table DTUT
(
	MaDTUT int primary key,
	TenLoaiDTUT varchar(20) not null,
	TL_MienHP decimal(5,2),
	check (TL_MienHP>=0 and TL_MienHP<=1)
)
alter table DTUT
alter column TenLoaiDTUT nvarchar(20) COLLATE Vietnamese_CI_AS not null
...
(
	...
	MaHuyen char(4),
	MaTinh char(4),
	DTUT int foreign key references DTUT(MaDTUT),
	MaKhoa char(4) foreign key references KHOA(MaKhoa),
	MaNganh char(4) foreign key references NGANH(MaNganh),
	foreign key (MaHuyen, MaTinh) references HUYEN(MaHuyen, MaTinh)
)

alter table HSSV
alter column HoTen nvarchar(40)
alter table HSSV
alter column GioiTinh nchar(3)
alter table HSSV
...
(
	...
	TenMH varchar(10) not null,
	LoaiTinChi char(3) not null,
	foreign key (LoaiTinChi) references TINCHI(LoaiTinChi),
	SoTiet int,
	MaNganh char(4) Foreign key references NGANH(MaNganh),
	MaKhoa char(4) Foreign key references KHOA(MaKhoa)
)
...
alter table DSMH
alter column TenMH nvarchar(100)

SELECT DSMH.MaMH, TenMH, DSMH.LoaiTinChi, SoTiet, MaNganh, MaKhoa, SoTiet / SoTietTinChi * SoTieNTinChi AS SoTien
FROM DSMH_Mo, DSMH, TINCHI WHERE DSMH_Mo.MaMH=DSMH.MaMH AND DSMH.LoaiTinChi=TINCHI.LoaiTinChi AND HocKy=1 AND Nam=2022

--Danh sach cac hoc ky va nam hoc--
delete from HOCKY
...
(
	...
	check (NgKetThuc > NgBatDau),
	HanDongHP smalldatetime,
	constraint HKNM_PK primary key (HocKy, Nam)
)

insert into HOCKY values
(1, 2022, '12/8/2022', '3/3/2023', '1/1/2023'),
(2, 2022, '4/3/2022', '9/9/2023', '1/6/2023'),
(1, 2021, '12/8/2021', '3/3/2022', '1/1/2022'),
(2, 2021, '4/3/2021', '9/9/2022', '1/6/2022')
...
(
	HocKy int,
	Nam int,
	foreign key (HocKy, Nam) references HOCKY(Hocky, Nam),
	MaMH char(10) foreign key references DSMH(MaMH),
	MaNganh char(4),
	MaKhoa char(4),
	GhiChu char(100),
	primary key (HocKy, Nam, MaMH)
)
ALTER TABLE CTHOC
ADD FOREIGN KEY (MaNganh) REFERENCES NGANH(MaNganh)
ALTER TABLE CTHOC
...
--Danh sach mon hoc mo--
select * from DSMH_Mo
create table DSMH_Mo
(
	HocKy int,
	Nam int,
	foreign key (HocKy, Nam) references HOCKY(Hocky, Nam),
...
CREATE TRIGGER TRG_DEL1_DSMH_Mo ON DSMH_Mo
FOR DELETE
AS
BEGIN
	IF EXISTS (SELECT * FROM DELETED I, CT_PHIEUDK CT WHERE I.MAMH = CT.MAMH)
	BEGIN
		PRINT N'Môn học đã được đăng ký'
		ROLLBACK TRAN
	END
END

CREATE TRIGGER TRG_DEL_CTHOC ON CTHOC
FOR DELETE
AS
BEGIN
	IF EXISTS (SELECT * FROM DELETED I, DSMH_MO MO WHERE I.MAMH = MO.MAMH AND I.NAM = MO.NAM AND I.HOCKY = MO.HOCKY)
	BEGIN
		PRINT N'Môn học có trong danh sách môn học mở'
		ROLLBACK TRAN
	END
END
DROP TRIGGER TRG_DEL_CTHOC

delete from DSMH_MO
SELECT * FROM DSMH_MO
SELECT * FROM CT_PHIEUDK

--Danh sach phieu DKHP--
create table DS_PHIEUDKHP
(
	SoPhieu int primary key,
	MSSV char(10) foreign key references HSSV(MSSV),
	NgLap smalldatetime,
	HocKy int,
	Nam int,
	constraint FK_DS_PHIEUDKHP foreign key (HocKy, Nam) references HOCKY(HocKy, Nam),
	SoTienDK money,
	SoTienDong money,
	SoTienConLai money
)
select * from DS_PHIEUDKHP
select * from CT_PHIEUDK
delete
from DS_PHIEUDKHP
delete
...
insert into DS_PHIEUDKHP values(5, 'SV00000003', '32/12/2022', 1, 2021, 0, 0, 0)

select * from DTUT
select * from DS_PHIEUDKHP
select * from CT_PHIEUDK
SELECT * FROM DS_PHIEUTHU
CREATE TRIGGER TRG_INS_CT_PHIEUDK ON CT_PHIEUDK
FOR INSERT
AS
BEGIN
	UPDATE DS_PHIEUDKHP
	SET SoTienDK = SoTienDK + (SELECT SUM(SoTiet / SoTietTinChi * SoTienTinChi) FROM inserted I, TINCHI TC, DSMH MH WHERE DS_PHIEUDKHP.SOPHIEU = I.SoPhieu AND I.MAMH = MH.MaMH AND MH.LOAITINCHI = TC.LoaiTinChi)
	WHERE SoPhieu IN (SELECT SoPhieu FROM inserted)

	UPDATE DS_PHIEUDKHP
	SET SoTienDong = SoTienDong + (SELECT 1 - TL_MIENHP FROM inserted I, DTUT DT, HSSV WHERE DS_PHIEUDKHP.SOPHIEU = I.SoPhieu AND DS_PHIEUDKHP.MSSV = HSSV.MSSV AND HSSV.DTUT = DT.MADTUT) * 
								(SELECT SUM(SoTiet / SoTietTinChi * SoTienTinChi) FROM inserted I, TINCHI TC, DSMH MH WHERE DS_PHIEUDKHP.SOPHIEU = I.SoPhieu AND I.MAMH = MH.MaMH AND MH.LOAITINCHI = TC.LoaiTinChi)
	WHERE SoPhieu IN (SELECT SoPhieu FROM inserted)

	UPDATE DS_PHIEUDKHP
	SET SoTienConLai = SoTienConLai + (SELECT 1 - TL_MIENHP FROM inserted I, DTUT DT, HSSV WHERE DS_PHIEUDKHP.SOPHIEU = I.SoPhieu AND DS_PHIEUDKHP.MSSV = HSSV.MSSV AND HSSV.DTUT = DT.MADTUT) * 
								(SELECT SUM(SoTiet / SoTietTinChi * SoTienTinChi) FROM inserted I, TINCHI TC, DSMH MH WHERE DS_PHIEUDKHP.SOPHIEU = I.SoPhieu AND I.MAMH = MH.MaMH AND MH.LOAITINCHI = TC.LoaiTinChi)
	WHERE SoPhieu IN (SELECT SoPhieu FROM inserted)

	UPDATE DS_PHIEUTHU
	SET TienThu = TienThu + (SELECT 1 - TL_MIENHP FROM inserted I, DTUT DT, HSSV, DS_PHIEUDKHP WHERE DS_PHIEUTHU.SOPHIEUDK = I.SoPhieu AND I.SoPhieu = DS_PHIEUDKHP.SoPhieu AND DS_PHIEUDKHP.MSSV = HSSV.MSSV AND HSSV.DTUT = DT.MADTUT) *
							(SELECT SUM(SoTiet / SoTietTinChi * SoTienTinChi) FROM inserted I, TINCHI TC, DSMH MH WHERE DS_PHIEUTHU.SOPHIEUDK = I.SoPhieu AND I.MAMH = MH.MaMH AND MH.LOAITINCHI = TC.LoaiTinChi)
	WHERE SoPhieuDK IN (SELECT SoPhieu FROM inserted)
END
drop trigger TRG_INS_CT_PHIEUDK
SELECT TL_MIENHP FROM DTUT DT, HSSV, DS_PHIEUDKHP WHERE DS_PHIEUDKHP.SOPHIEU = 1 AND DS_PHIEUDKHP.MSSV = HSSV.MSSV AND HSSV.DTUT = DT.MADTUT
SELECT TL_MIENHP FROM DTUT DT, HSSV, DS_PHIEUDKHP, DS_PHIEUTHU WHERE DS_PHIEUTHU.SOPHIEUDK = 1 AND DS_PHIEUDKHP.MSSV = HSSV.MSSV AND HSSV.DTUT = DT.MADTUT

select * from DTUT
select * from DS_PHIEUDKHP
select * from CT_PHIEUDK
select * from DS_PHIEUTHU where SOPHIEUDK = 1

delete from DS_PHIEUTHU
select * from hocky
alter table DS_PHIEUDKHP
alter column SoPhieu char(4)
...
(
	...
	MAMH char(10),
	SoTC int,
	constraint PK_CT primary key (SoPhieu, MAMH),
	constraint FK_CT_PHIEUDKHP_SP foreign key (SoPhieu) references DS_PHIEUDKHP(SoPhieu),
	constraint FK_CT_PHIEUDKHP_MH foreign key (MAMH) references DSMH(MaMH),
)
drop table CT_PHIEUDK


--DS phieu thu hoc phi
create table DS_PHIEUTHU
(
 SoPhieuThu int primary key,
 SoPhieuDK int foreign key references DS_PHIEUDKHP(SoPhieu),
 NgLap smalldatetime,
 TienThu money
)
...
select MATINH, TenTinh COLLATE utf-8 from TINH
...
Select TenHuyen from HUYEN h join TINH t on h.MaTinh = t.MaTinh where t.TenTinh like 'B?c Ninh'

insert into KHOA values
('K001','Khoa học máy tính'),
('K002','Khoa học và kỹ thuật thông tin'),
('K003','Kỹ thuật máy tính'),
('K004','Công nghệ phần mềm'),
('K005','Hệ thống thông tin'),
('K006','Mạng máy tính và truyền thông')

insert into NGANH values
('N001','Thị giác máy tính', 'K001'),
('N002','Xử lí ngôn ngữ tự nhiên', 'K001'),
('N003','Công nghệ thông tin', 'K002'),
('N004','Khoa học dữ liệu', 'K002'),
('N005','IOT', 'K003'),
('N006','Công nghệ robot', 'K003'),
('N007','Phát triển phần mềm', 'K004'),
('N008','Phát triển game', 'K004'),
('N009','Hệ thống thông tin', 'K005'),
('N010','Thương mại điện tử', 'K005'),
('N011','An toàn thông tin', 'K006'),
('N012','Truyền thông dữ liệu', 'K006')
...
create table LOGIN_SINHVIEN
(
	TenDangNhap char(50) primary key,
	MatKhau varchar(100),
	MSSV char(10) foreign key references HSSV(MSSV)
)
select * from LOGIN_SINHVIEN
create table LOGIN_PDT
(
	TenDangNhap char(50) primary key,
	MatKhau varchar(100)
)

insert into HSSV values 
('SV00000001', 'Tô Anh Phát', '19/09/2003', 'Nam', 'H705', 'T029', 0, 'K001', 'N001'),
('SV00000002', 'Trương Thanh Minh', '19/09/2003', 'Nam', 'H705', 'T029', 0, 'K001', 'N001'),
('SV00000003', N'Lê Chí Cường', '19/09/2003', 'Nam', 'H705', 'T029', 0, 'K001', 'N001')

insert into HSSV values
('SV00000004', 'Lê Chí Cường', '19/09/2003', 'Nam', 'H705', 'T029', 0, 'K001', 'N001')

insert into LOGIN_SINHVIEN values
('ttm', '123456789', 'SV00000002'),
('tap', '123456789', 'SV00000001'),
('lcc', '123456789', 'SV00000003')
